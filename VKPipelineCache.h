...
public:
    ...
    bool saveGraphicsPiplineCache(const std::string& file) {
        size_t cacheSize = 0;
        //get current pipeline cache data size
        ...
        //get currtent pipeline cache data ,save into cacheData,
    ...
    //validated PCO file head data
    bool isValidCacheData(const std::string& filename, const char* buffer, uint32_t size) {
        if (size > 32) {
            uint32_t header = 0;
            uint32_t version = 0;
            uint32_t vendor = 0;
